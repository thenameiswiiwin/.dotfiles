...
# Given an array of decimal numbers, print all unicode codepoints.
function print-decimal-unicode-range() {
	...
	for decimalCode in "${originalSequence[@]}"; do
		...
		if [[ $((counter % wrapAt)) -eq 0 ]]; then
			...
			if [[ "$counter" -ne "$sequenceLength" ]]; then
				printf "%b\\n" "$line"
			fi
...
# Given an array of hexadecimal ranges, print all unicode codepoints.
...
# Test the font ranges for Nerd Fonts categories
function test-fonts() {
...
# Execute the test-fonts function
test-fonts
